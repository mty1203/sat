# -*- coding: utf-8 -*-
"""
Created on Fri Jun 21 03:55:12 2024
...
def generate_even_random_numbers(num_variables, count=3):
    # Ensure that count does not exceed half of num_variables if num_variables is less than 6
    count = min(count, num_variables // 2)
    
    # Generate even numbers within the range 0 to num_variables
    even_numbers = np.arange(0, num_variables , 2)
    
    # Randomly select 'count' even numbers from the generated array
    random_even_numbers = np.random.choice(even_numbers, size=count, replace=False)
    
    return random_even_numbers

def simulated_annealing(J, h, num_variables, variable_clause_map, max_iterations=10000, initial_temperature=1.0, final_temperature=0.01):
...
#         flip_indices = np.random.choice(range(num_variables), size=group_size, replace=False)
        ...
        # Flip the spins in groups and update auxiliary variables
        variables_flipped, auxiliary_vars_flipped = flip_and_update_vector(variables_flipped, auxiliary_vars_flipped, flip_indices, num_variables, variable_clause_map)
...
if __name__ == '__main__':
    ...
    # Run Simulated Annealing algorithm to solve the Ising problem
    best_variables, best_auxiliary_vars, best_energy, assignments = simulated_annealing(J, h, num_variables, variable_clause_map)
    print(assignments)
    result = evaluate_cnf(formula, assignments)
